...
def most_goals(players: list):
    return max(players, key = lambda goal: goal.goals).name
def most_points(team:list):
    player = max(team, key = lambda score: (score.goals+ score.passes))
    return player.name, player.number
def least_minutes(team:list):
    return min(team, key = lambda time: time.minutes)
